...
- **札の絵**: 48枚それぞれを軽量なインライン SVG で手続き的に描画する
  (`components/CardArt.tsx`)。月ごとの植物モチーフ+種別のオーバーレイ
  (光札の固有絵・タネの動物・短冊のリボン)の合成で、画像アセットを持たない
- **月の視認性**: 札の左上に「n月」バッジを常時表示し、下部に札名を添える
- **手札の扇**: 自分の手札は緩い扇状(中央を頂点に回転±・弧状の沈み込み)で
  重ねて表示する。相手の手札・場・取り札は従来のグリッド
- **ドラッグ打牌**: 手札は場へのドラッグ(スライド)で出せる。
  合わせ札が2枚あるときはドロップした場札を合わせ先として解釈し、
  場の空きに落とした場合は従来のタップ選択にフォールバック。
  タップ操作(1タップで自動合わせ)も並存させる。ドラッグは Pointer Events で
  実装し、タップとの判別は移動量(8px)で行う
